...
export default function ProductCard({ product, category }) {
  ...
  return (
    <div className="group rounded-2xl sm:rounded-3xl bg-white border border-brand-100/70 shadow-sm hover:shadow-luxury-hover hover:border-brand-300 transition-all duration-300 flex flex-col justify-between overflow-hidden">
      ...
      <div className="relative aspect-[4/3] w-full overflow-hidden bg-cream-100 shimmer-card">
        ...
        <div className="absolute top-3 left-3 flex flex-wrap gap-1">
          {product.tags && product.tags.map((tag, idx) => (
            <span
              key={idx}
              className="px-2 py-0.5 rounded-full bg-white/90 backdrop-blur-md text-[10px] font-semibold text-brand-900 border border-brand-100/80 shadow-xs"
            >
              {tag}
            </span>
          ))}
        ...
        {!category.hasSizes && category.fixedSizeLabel && (
          <div className="absolute bottom-2.5 left-2.5 px-2.5 py-0.5 rounded-md bg-charcoal-900/75 backdrop-blur-md text-white text-[11px] font-medium">
      ...
      <div className="p-5 sm:p-6 flex flex-col flex-grow justify-between">
        <div>
          <div className="flex items-start justify-between gap-2 mb-1.5">
            <h4 className="font-serif text-lg font-bold text-charcoal-900 group-hover:text-brand-800 transition-colors">
              {product.name}
            </h4>
          </div>

          <p className="text-xs text-charcoal-600 font-light leading-relaxed mb-4 line-clamp-2">
            {product.description}
          </p>
        </div>

        {/* Sizing & Add-ons Controls */}
        <div className="space-y-3 pt-3 border-t border-brand-50">
          
          {/* Dual Size Selector (Half vs Full) */}
          {category.hasSizes && (
            <div className="flex items-center justify-between gap-2">
              <span className="text-[11px] font-medium text-charcoal-500">Size:</span>
              <div className="flex bg-cream-100 p-0.5 rounded-xl border border-brand-100">
                <button
                  type="button"
          ...
          {category.heartShapeAvailable && (
            <div className="flex items-center justify-between">
              <span className="text-[11px] text-charcoal-600 flex items-center gap-1">
          ...
          <div className="flex items-center justify-between pt-2">
            <div>
              <span className="text-[10px] text-charcoal-400 uppercase tracking-wider block">Price</span>
            ...
            <a
              ...
              className="inline-flex items-center gap-1.5 px-4 py-2 rounded-full bg-brand-700 hover:bg-brand-800 text-white font-semibold text-xs tracking-wide transition-all shadow-sm shadow-brand-700/20 hover:shadow-md hover:-translate-y-0.5 active:translate-y-0"
            >
